...
public class RemoveElev extends JDialog {
	//Attributes
	private static final long serialVersionUID = 1L;
	private final JPanel contentPanel = new JPanel();

	//Create the Dialog
	public RemoveElev(MainMenu menu, JDialog caller, String elev) {
		...
		//Initial SetUp
		JLabel lblSeguroQueDeseas = new JLabel("Seguro que deseas eliminar este alumno?");
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (cont.remove(RemoveElev.this, elev)) {
					cont.rightMessage(caller, "Alumno eliminado correctamente!");
				} else {
					cont.errorMessage(caller, "Internal Error!");
				}
				caller.dispose();
				menu.setEnabled(true);
				cont.setStudents(menu);
				dispose();
			}
		});
